    ...
    foreach ($allRows as $row)
    {
        ...
        if(!empty($candidateId) && !empty($jobOrderId))
        {
            ...
            if($changedEntityId)
            {
                ...
                @shell_exec('echo "'.$row->ApplicationID.'", "'.$row->ContactID.'", "'.$row->JobOrderID.'", "'.$row->CompanyID.'", "'.$row->FullName.'", "'.$candidateId.'", "'.$row->jobOrderId.'", "'.$changedEntityId.'" >> JobSubmission_log.txt');
            
            }else{
                
                @shell_exec('echo "'.$row->ApplicationID.'", "'.$row->ContactID.'", "'.$row->JobOrderID.'", "'.$row->CompanyID.'", "'.$row->FullName.'", "'.$candidateId.'", "'.$row->jobOrderId.'", "'.$changedEntityId.'" >> NotLoadedJobSubmission_log.txt');
            }
...
function uploadDataToBullhorn($row, $candidateId, $jobOrderId) : int
{
    ...
    $requestBody = [
        ...
        // 'dateAdded' => strtotime($row->DateCreated) * 1000,
        // 'dateLastModified' => strtotime($row->DateCreated) * 1000,
        "jobOrder" => [
            'id' => $jobOrderId
        ],
        "status" => "Submitted"
